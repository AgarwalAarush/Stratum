    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-[13px] font-semibold text-[var(--text)] group-hover:text-[var(--accent)]">{item.ticker}</span>
          <span className="text-[13px] font-normal text-[var(--text-dim)] truncate">{item.companyName}</span>
          {!isUpcoming && (
            <span
              className={`text-[11px] font-semibold ml-auto ${beat ? 'text-[var(--green)]' : 'text-[var(--red)]'}`}
            >
              {beat ? 'Beat ↑' : 'Miss ↓'}
            </span>
          )}
