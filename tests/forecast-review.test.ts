...
import { forecastsAreApproved, reviewedForecasts, forecastCategory } from '../lib/markets/forecast-review.ts'
...
test('security price returns cannot masquerade as operating evidence', () => {
  for (const metric of ['Fund price return percentage','Stock return','ETF total return','Share price target'])
    assert.equal(forecastCategory({metric}),'market_return')
  for (const metric of ['Reported operating cash flow dollars','Revenue growth','Return on equity','FRED:CPIAUCSL','Oil price per barrel'])
    assert.equal(forecastCategory({metric}),'economic')
})
