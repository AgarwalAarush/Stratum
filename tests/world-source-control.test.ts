...
test('source-control migration makes contracts mandatory for newly governed observations', async () => {
  ...
  assert.match(controlService, /ensureDeclaredMarketDomainPacks/)
  assert.match(controlService, /Persisted market domain .* newer than this worker release/)
  assert.match(adapters, /listWorldSourceAdapters/)
  assert.match(jobs, /domain \$\{adapter\.domain\} is not active/)
  const worker = await readFile(new URL('../scripts/markets-worker.ts', import.meta.url), 'utf8')
  assert.match(worker, /ensureDeclaredMarketDomainPacks/)
  assert.match(worker, /declared_domain_coverage_review_queued/)
  ...
  assert.match(controlPanel, /Queue recurring-source scout/)
  assert.match(controlPanel, /Queue broad research scout/)
  assert.match(controlRoute, /'scout-broad-research'/)
  assert.match(controlPanel, /Approve reviewed contract/)
  assert.match(controlPanel, /function isApprovedSource\(status: string\)/)
  assert.match(controlPanel, /return status === 'approved'/)
  assert.match(controlPanel, /function sourceCoverage[\s\S]*?filter\(\(source\) => isApprovedSource\(source\.status\)\)/)
  ...
  assert.match(controlPanel, /prioritizeWorldObservationProposals\(workspace\.observationProposals, workspace\.researchFrontiers, selectedDomain\?\.id\)/)
  assert.match(controlPanel, /Addresses research frontier/)
  ...
  assert.match(controlPanel, /policy auto-accept or human review/)
  assert.match(controlPanel, /Accept as observation/)
  assert.match(controlPanel, /review-observation-proposal/)
  assert.match(controlPanel, /Run proposal auto-accept now/)
