...
test('worker startup recovers stale claimed jobs after a hard stop', async () => {
  ...
  assert.match(agentJobsSource, /export async function recoverInterruptedAgentJobs/)
  assert.match(agentJobsSource, /\.eq\('claimed_by', claimedBy\)/)
  assert.match(agentJobsSource, /45 \* 60 \* 1_000/)
  assert.match(agentJobsSource, /agentJobStaleAfterMs\(jobType, defaultStaleAfterMs\)/)
  assert.match(agentJobsSource, /export async function supersedeQueuedRoutineAgentJobs/)
  assert.match(agentJobsSource, /Superseded by newer routine snapshot work after queue backlog/)
  assert.match(agentJobsSource, /status: 'queued'/)
  assert.match(agentJobsSource, /Recovered after the worker stopped/)
  assert.match(workerSource, /await recoverInterruptedAgentJobs\(workerId\)/)
  assert.match(workerSource, /await recoverStaleAgentJobs\(\)/)
  assert.match(workerSource, /nextRecoveryAt/)
  assert.match(workerSource, /supersedeQueuedRoutineAgentJobs/)
  assert.match(workerSource, /routine_queue_superseded/)
