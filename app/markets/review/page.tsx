...
export default async function MarketsReviewPage() {
  ...
  return <div className="owner-review-page">
    <header className="owner-review-header">
      ...
      <aside><span>Weekly capacity</span><strong>{queue.length} / 10 open</strong><small>{snapshot.deferredReviewCount ? `${snapshot.deferredReviewCount} lower-priority items are retained without demanding attention.` : 'Urgent falsifiers can still interrupt.'}</small></aside>
    </header>
