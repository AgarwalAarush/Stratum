import { decisionHeadline, decisionIsBlocked } from './recommendation-display.ts'
...
export function renderInvestmentNewsletter(input: {
  ...
  const blocks = [
    ...
    ...input.recommendations.map(r => {
      const details = !decisionIsBlocked(r) && ['buy', 'add', 'trim', 'sell'].includes(r.action)
  ...
  const cards = input.recommendations.map(r => {
    const details = !decisionIsBlocked(r) && ['buy', 'add', 'trim', 'sell'].includes(r.action)
      ? `<p><b>Thesis</b> ${escape(r.thesis)}</p><p><b>Counter-thesis</b> ${escape(r.counterThesis)}</p><p><b>Entry</b> ${escape(r.entry.condition)}</p><p><b>Invalidation</b> ${escape(r.invalidation.join('; '))}</p>` : ''
